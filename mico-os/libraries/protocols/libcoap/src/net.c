...
#if defined(WITH_POSIX) || defined(WITH_MICO)
...
coap_send_impl(coap_context_t *context, 
  ...
  char addr[48]={0};
  coap_print_addr(dst,addr,48);

  bytes_written = sendto( context->sockfd, pdu->hdr, pdu->length, 0,
			  &dst->addr.sa, dst->size);


//  warn("[test] send to  fd[%d] pdu[%d] size[%d] %s \r\ndata: %s",
//        context->sockfd,pdu->length,bytes_written,addr, pdu->data);
...
//debug("****retransmission payload:%s  ",node->pdu->data);
char addr[48]={0};
coap_print_addr(&node->remote,addr,48);
debug("****retransmission addr:%s  ",addr);
  ...
  if (node->pdu->hdr->code >= 64) {
    ...
    // edit by 2017.11.6 not remove observe
    // solve program that observes fail after 10 minutes
...
//  debug("recive data len %d", bytes_read);
  node = coap_new_node();
  if ( !node )
    goto error_early;
