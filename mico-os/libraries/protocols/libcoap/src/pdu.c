...
coap_add_option(coap_pdu_t *pdu, unsigned short type, unsigned int len, const unsigned char *data) {
  ...
  if (!optsize) {
    ...
    debug("pdu->max_size: %d, pdu->length: %d, data: %s, len: %d", pdu->max_size, pdu->length, data, len);
    /* error */
    return 0;
  } else {
    pdu->max_delta = type;
...
coap_add_data(coap_pdu_t *pdu, unsigned int len, const unsigned char *data) {
  ...
  if (pdu->length + len + 1 > pdu->max_size) {
    warn("coap_add_data: cannot add: data too large for PDU[%d %d %d]\n",pdu->length,len,pdu->max_size);
    assert(pdu->data == NULL);
    return 0;
  }
...
//    if (strlen(pdu->data) > length)
//    {
//        pdu->data[length] = 0;
//    }
//    debug("coap_pdu_parse: %s", pdu->data);
  }
