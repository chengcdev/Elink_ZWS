...
static mico_worker_thread_t button_worker_thread;

typedef void (*multi_click_cb)( int arg );
static multi_click_cb button_multi_click_cb = NULL;
...
static OSStatus system_button_worker( void *arg )
{

    if (button_multi_click_cb != NULL)
    {
        (button_multi_click_cb)(*(int *)arg);
    }
    return kNoErr;
}
...
//    return system_config_mode_worker;
#endif
}

OSStatus mico_system_button_work_start( void *arg )
{
    return mico_rtos_send_asynchronous_event( &button_worker_thread, system_button_worker, arg );
}


OSStatus mico_system_multi_click_cb( multi_click_cb multi_click_callback )
{
    button_multi_click_cb = multi_click_callback;
    return kNoErr;
}
...
#endif
  err = mico_rtos_create_worker_thread( &button_worker_thread, MICO_APPLICATION_PRIORITY, 0x500, 1 );
  require_noerr_string( err, exit, "ERROR: Unable to start the button worker thread." );
