...
/*-----------------------------------------------------------*/
#ifdef MXCHIP
void event_rx_cb(xQueueHandle queue);
#endif
...
/*-----------------------------------------------------------*/
        ...
        void *pxReturn;
                ...
                taskENTER_CRITICAL();
                {
                        if( ( ( xQueueHandle ) xSemaphore )->uxQueueType == queueQUEUE_IS_MUTEX )
                        {
                                pxReturn = ( void * ) ( ( xQueueHandle ) xSemaphore )->pxMutexHolder;
                        }
                        else
                        {
                                pxReturn = NULL;
	...
	for( ;; )
	{
		...
		if( xTaskCheckForTimeOut( &xTimeOut, &xTicksToWait ) == pdFALSE )
		{
			if( prvIsQueueEmpty( pxQueue ) != pdFALSE )
			{
				...
				{
                    portYIELD_WITHIN_API();
#if ( INCLUDE_vTaskForceAwake == 1 )
                    if( xTaskForceAwakePending() )
                    {
                        /* Support for for vTaskForceAwake */
                        prvLockQueue( pxQueue );
                        /* If this task was removed externally from the event list while waiting for a message in a queue
                                         * we will return with an error, that a queue is empty
                                         * a vTaskForceAwake function will deattach task from the queue and place it from suspended or delayed list
                                         * list back to a ready list
                                         */

                        /* If a queue is still empty but and force awake is pending
                         * return that queue is empty */
                        if( prvIsQueueEmpty( pxQueue ) )
                        {
                            xTaskForceAwakeClearPending();
                            prvUnlockQueue( pxQueue );
                            return errQUEUE_EMPTY;
                        }
                        prvUnlockQueue( pxQueue );
                    }
#endif /* INCLUDE_vTaskForceAwake */
