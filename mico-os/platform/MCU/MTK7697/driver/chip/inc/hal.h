 ...
 *
 * - \b Introduction.\n
 *   MediaTek LinkIt™ SDK v3 hardware abstraction layer (HAL) driver is embedded software ensuring maximized portability across IoT portfolio designed for MediaTek LinkIt for real-time OS (RTOS) development platform.\n
 *   The HAL driver includes:\n
 *   - Drivers for the peripheral modules, such as UART\I2C.\n
 *   - Drivers for the system modules, such as CACHE\MPU\FLASH.\n
 *   - APIs of the modules.\n
 *   .
 ...
 *   The HAL layer provides a generic set of APIs to interact with the upper layers, such as application, middleware and the OS.The APIs are defined interfaces for the MediaTek IoT portfolio, so the upper layer written with the HAL API is portable across the MediaTek IoT platforms.\n
 *   The HAL driver covers rich modules, including all peripheral modules, such as UART, I2C, SPI. It also includes number of system modules, such as CACHE, MPU, FLASH, DMA.\n
 ...
 *    - Driver: All common driver files, such as board driver, HAL driver, CMSIS.\n
 ...
 *   For example, call the #hal_gpt_delay_us() API to apply a microsecond time delay in the GPT. The GPT HAL driver will poll the GPT hardware, if the time delay is reached.\n
