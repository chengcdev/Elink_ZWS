 ...
 *    The I2S interface, which supports the I2S slave mode only, is used to connect to an external audio codec. Bit clock and word select are derived from external devices when the slave mode is configured.
 *  - The user can set the initial type to #HAL_I2S_TYPE_EXTERNAL_MODE in #hal_i2s_init() function when connecting an external codec component. \n
 *    An interrupt will be triggered when the data is ready to be transferred using the I2S interface. A user defined callback function is called to handle data transfer in the NVIC ISR. \n
 ...
 *      The user should call #hal_i2s_setup_tx_vfifo() and #hal_i2s_setup_rx_vfifo() functions to initialize the DMA hardware
 ...
 *
 * @image html hal_i2s_dma_interrupt_archi.png
 *
 ...
 *
 *      uint32_t tx_buffer[TX_BUFFER_LENGTH];
 *      uint32_t rx_buffer[RX_BUFFER_LENGTH];
 *
 ...
 *          }
 *
 *          //initialize the VFIFO
 *          memset(tx_buffer, 0, sizeof tx_buffer);
 *          memset(rx_buffer, 0, sizeof rx_buffer);
 *
 *          //The I2S interface can support the I2S slave mode only
...
typedef enum {
    HAL_I2S_TX_MONO_DUPLICATE_DISABLE = 0,  /**< Right channel of the TX, sends all 0. */
...
typedef enum {
    ...
    HAL_I2S_RX_DOWN_RATE_ENABLE  = 1   /**< Actual sampling rate of the I2S RX is half of the original sampling rate.*/
...
typedef struct {
    ...
    uint8_t                            lr_swap;             /**< Enable or disable data swapping between right and left channels of the I2S link by setting this value.
    ...
    uint8_t                            word_select_inverse; /**< Enable or disable word select clock inverting of the I2S link by setting this value.
 ...
 * @return    #HAL_I2S_STATUS_ERROR, if one of the I2S links is still available. Call #hal_i2s_disable_tx(), #hal_i2s_disable_rx() and #hal_i2s_disable_audio_top() to disable the TX/RX links before calling this function. \n
 *            #HAL_I2S_STATUS_OK, if the operation is successful.
 * @sa        #hal_i2s_init()
 */
#else
 ...
 *            #HAL_I2S_STATUS_ERROR, if one of the I2S links is still available. Call #hal_i2s_disable_tx(), #hal_i2s_disable_rx() and #hal_i2s_disable_audio_top() to disable the TX/RX links before calling this function. \n
 *            #HAL_I2S_STATUS_OK, if the operation is successful.
 ...
 * @param[out] sample_count is the available free space in the TX VFIFO.
