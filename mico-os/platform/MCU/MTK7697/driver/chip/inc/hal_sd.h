 ...
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 ...
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
 *  - Step2: Call #hal_sd_register_card_detection_callback() to register the SD card detection callback.
 ...
 *       hal_pinmux_set_function(gpio_pin, function_index);//Set the GPIO pinmux.
 *       ret = hal_sd_register_card_detection_callback(HAL_SD_PORT_0, sd_detection_callback, NULL);
 *       if (HAL_SD_STATUS_OK != ret) {
 *          //Error handler.
 *       }
