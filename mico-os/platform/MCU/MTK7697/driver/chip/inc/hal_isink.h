 ...
 *  - Sample code:
 *    @code
 *       ret = hal_isink_init(HAL_ISINK_CHANNEL_0); //Initialize the ISINK clock source.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 *       ret = hal_isink_set_clock_source(HAL_ISINK_CHANNEL_0,HAL_ISINK_CLOCK_SOURCE_2MHZ); //Set the ISINK clock source.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 *       ret = hal_isink_set_mode(HAL_ISINK_CHANNEL_0,HAL_ISINK_MODE_REGISTER) ; //Set the ISINK register mode.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 *       ret = hal_isink_set_step_current(HAL_ISINK_CHANNEL_0,HAL_ISINK_OUTPUT_CURRENT_4_MA); //Set the ISINK to 4mA output current.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 *       ret = hal_isink_get_running_status(HAL_ISINK_CHANNEL_0,&status) ; //Get the running status of the ISINK.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 *       ret = hal_isink_deinit(HAL_ISINK_CHANNEL_0); //Deinitialize the ISINK module.
 *       if(HAL_ISINK_STATUS_OK != ret) {
 *             //Error handler
 *       }
 ...
 *            If the return value is #HAL_ISINK_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
 * @sa  hal_isink_deinit()
 */
 ...
 * @param[in]  channel is the ISINK channel number. For more details about the parameter, please refer to #hal_isink_channel_t.
 * @param[in]  source_clock is the ISINK clock source setting. For more details about the parameter, please refer to #hal_isink_source_clock_t.
 * @return     Indicates whether this function call is successful or not.
 *            If the return value is #HAL_ISINK_STATUS_OK, the operation completed successfully.
 *            If the return value is #HAL_ISINK_STATUS_INVALID_PARAMETER, a wrong parameter is given. The parameter needs to be verified.
