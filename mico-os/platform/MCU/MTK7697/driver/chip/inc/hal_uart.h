 ...
 * @addtogroup UART
 * @{
 * This section introduces the UART APIs including terms and acronyms, supported features, software architecture, details on how to use this driver, UART function groups, enums, structures and functions.
 *
 ...
 *           hal_pinmux_set_function(gpio_pin, 1);
 *
 *           // configure RXD pin
 *           gpio_pin = HAL_GPIO_1;
 *           hal_gpio_init(gpio_pin);
 ...
 *  - Step2: call hal_uart_set_dma() to configure the VFIFO DMA hardware.
...
hal_uart_status_t hal_uart_dma_init(hal_uart_port_t uart_port, const hal_uart_dma_config_t *uart_dma_config);
hal_uart_status_t hal_uart_dma_set_tx_interrupt(hal_uart_port_t uart_port, uint8_t enable);
hal_uart_status_t hal_uart_dma_set_rx_interrupt(hal_uart_port_t uart_port, uint8_t enable);
hal_uart_status_t hal_uart_dma_set_tx_threshold(hal_uart_port_t uart_port, uint32_t threshold);
hal_uart_status_t hal_uart_dma_set_rx_threshold(hal_uart_port_t uart_port, uint32_t threshold);
hal_uart_status_t hal_uart_dma_set_tx_timeout(hal_uart_port_t uart_port, uint32_t timeout);
hal_uart_status_t hal_uart_dma_set_rx_timeout(hal_uart_port_t uart_port, uint32_t timeout);
hal_uart_status_t hal_uart_dma_register_tx_callback(hal_uart_port_t uart_port,
        void (*callback)(void));
hal_uart_status_t hal_uart_dma_register_rx_callback(hal_uart_port_t uart_port,
        void (*callback)(void));
uint32_t hal_uart_dma_tx(hal_uart_port_t uart_port, const uint8_t *data, uint32_t size);
uint32_t hal_uart_dma_rx(hal_uart_port_t uart_port, uint8_t *buffer, uint32_t size);
