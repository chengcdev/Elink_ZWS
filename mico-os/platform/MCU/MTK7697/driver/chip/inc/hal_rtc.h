 ...
 * @addtogroup RTC
 * @{
 * This section introduces the RTC APIs including terms and acronyms,
...
*   Call #hal_rtc_set_time_callback() to register a callback to receive RTC current time change notification.
*   Call #hal_rtc_set_time_notification_period() to register a callback to set the desired notification period.
*   Then your callback function will be periodically executed according to the RTC current time change notification period you set.
...
*   There are several spare registers in the RTC to store data that won't be cleared during a power off, except when the battery is removed.
*
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware, if it hasn't been called after the power went on.
*  - Step2: Call #hal_rtc_set_time() to set the RTC current time.
*  - sample code:
*    @code
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware if it has not been called after the power went on.
*  - Step2: Call #hal_rtc_get_time() to get RTC current time.
*  - sample code:
*    @code
*       #define BASE_YEAR 2000
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware if it has not been called after the power went on.
*  - Step2: Call #hal_rtc_set_time_notification_period() to set a specific RTC current time change notification period.
*  - Step3: Call #hal_rtc_set_time_callback() to register a user callback function to handle the RTC current time change, then when a
...
*       {
*           // *user_data is g_user_data in this case.
...
*           ret = hal_rtc_set_time_notification_period(period);
*           if(HAL_RTC_STATUS_OK != ret) {
*               //error handling
*           }
*
*           //Register a user callback function to handle the RTC current time change.
*           ret = hal_rtc_set_time_callback(time_change_cb, &g_user_data);
*           if(HAL_RTC_STATUS_OK != ret) {
*               //error handling
*           }
*       }
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware if it has not been called after the power went on.
*  - Step2: Call #hal_rtc_set_time() to set the RTC alarm time.
*  - sample code:
*    @code
*       #define BASE_YEAR 2000
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware if it has not been called after the power went on.
*  - Step2: Call #hal_rtc_get_alarm() to get the RTC alarm time.
*  - sample code:
*    @code
...
*  - Step1: Call #hal_rtc_init() to initialize the RTC hardware if it has not been called after the power went on.
*  - Step2: Call #hal_rtc_set_alarm_callback() to register a user callback function to handle the RTC alarm.
*  - Step3: Call #hal_rtc_enable_alarm() to enable an alarm notification. A callback function is triggered in an
...
*           ret = hal_rtc_set_alarm_callback(alarm_handle_cb, &g_user_data);
*           if(HAL_RTC_STATUS_OK != ret) {
*               //error handling
*           }
*
...
/** @brief  This macro defines a magic number used in #hal_rtc_sleep() function.
...
/** @brief RTC status */
typedef enum {
    HAL_RTC_STATUS_ERROR = -2,                              /**< The RTC function error. */
    HAL_RTC_STATUS_INVALID_PARAM = -1,                      /**< The RTC error invalid parameter. */
    HAL_RTC_STATUS_OK = 0                                   /**< The RTC function OK. */
} hal_rtc_status_t;
...
typedef enum {
    ...
    HAL_RTC_TIME_NOTIFICATION_EVERY_SECOND = 1,             /**< Execute callback function set by hal_rtc_set_time_notification_period() for every second. */
    HAL_RTC_TIME_NOTIFICATION_EVERY_MINUTE = 2,             /**< Execute callback function set by hal_rtc_set_time_notification_period() for every minute. */
    HAL_RTC_TIME_NOTIFICATION_EVERY_HOUR = 3,               /**< Execute callback function set by hal_rtc_set_time_notification_period() for every hour. */
    HAL_RTC_TIME_NOTIFICATION_EVERY_DAY = 4,                /**< Execute callback function set by hal_rtc_set_time_notification_period() for every day. */
    HAL_RTC_TIME_NOTIFICATION_EVERY_MONTH = 5,              /**< Execute callback function set by hal_rtc_set_time_notification_period() for every month. */
    HAL_RTC_TIME_NOTIFICATION_EVERY_YEAR = 6                /**< Execute callback function set by hal_rtc_set_time_notification_period() for every year. */
 ...
 * @return #HAL_RTC_STATUS_OK means this function completed successfully.
 */
 ...
 * @param[in] time is the pointer to a #hal_rtc_time_t structure that contains the date and time settings that will be set to the RTC current time.
 * @return #HAL_RTC_STATUS_INVALID_PARAM means an invalid time parameter is given.
 *         #HAL_RTC_STATUS_OK means this function completed successfully.
 * @par Example
 * Sample code please refer to "Set RTC current time" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
 ...
 * @return #HAL_RTC_STATUS_OK means this function completed successfully.
 * @par Example
 * Sample code please refer to "Get RTC current time" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
hal_rtc_status_t hal_rtc_get_time(hal_rtc_time_t *time);

#ifdef HAL_RTC_FEATURE_TIME_CALLBACK
/**
 * @brief Set the RTC current time change notification period.
 * @param[in] period is the expected time for the RTC current time change notification period.
 * @return #HAL_RTC_STATUS_INVALID_PARAM means an invalid period parameter is given.
 *         #HAL_RTC_STATUS_OK means this function completed successfully.
 * @sa #hal_rtc_set_time_callback()
 * @par Example
 * Sample code please refer to "Register a user callback function for handling RTC current time change" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
 ...
 * @param[in] time is the pointer to a hal_rtc_time_t structure that contains the date and time settings that will be set to the RTC alarm time.
 * @return #HAL_RTC_STATUS_INVALID_PARAM means an invalid alarm time parameter is given.
 *         #HAL_RTC_STATUS_OK means this function completed successfully.
 * @par Example
 * Sample code please refer to "Set RTC current time" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
 ...
 * @param[out] time is the pointer to a hal_rtc_time_t structure to store the date and time settings received from the RTC alarm time.
 * @return #HAL_RTC_STATUS_OK means this function completed successfully.
 * @par Example
 * Sample code please refer to "Get RTC alarm time" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
 ...
 * @return #HAL_RTC_STATUS_OK means this function completed successfully.
 * @par Example
 * Sample code please refer to "Register a user callback function for handling alarm" in @ref HAL_RTC_Driver_Usage_Chapter.
 */
...
#ifdef HAL_RTC_FEATURE_TIME_CALLBACK
/**
 * @brief Set the RTC time notification callback, and the callback executing period is set by hal_rtc_set_time_notification_period().
 * @param[in] callback_function is the callback function given by user.
...
#ifdef HAL_RTC_FEATURE_SLEEP
/**
 * @brief Put the system into sleep. This function should not be called from ISR and the caller is
 *        responsible for making sure the ISR masks won't block the expecting ISRs, which will then
 *        put the system into sleep and not able to wake up.
 ...
 *         #HAL_RTC_STATUS_INVALID_PARAM means an invalid magic parameter is given.
 *         #HAL_RTC_STATUS_OK means this function completed successfully.
 */
...
/**
 * @brief Set the ticks that are adjusted to the RTC current time once. Note, this function should be executed only once per second.
...
/**
 * @brief Get the ticks that are adjusted to the RTC current time once. Note, ticks will equal to zero after executing this function if one shot calibration is finished.
 * @param[in] ticks is the ticks that are adjusted to the RTC current time. The valid value is in a range from -2048(-62.5ms) to 2045(62.4ms). The unit is about 30.52us (1s/32768).
 * @return #HAL_RTC_STATUS_OK means this function completed successfully.
 ...
 *       hal_rtc_status_t ret;
 *
 *       //write 3 bytes from buf to RTC spare registers byte 2~4.
 *       ret = hal_rtc_set_data(2, buf, 3);
 *       if(HAL_RTC_STATUS_OK != ret) {
 *           //error handling
 *       }
 * @endcode
 ...
 *       hal_rtc_status_t ret;
 *
 *       //write 3 bytes zero data to RTC spare registers byte 2~4.
 *       ret = hal_rtc_clear_data(2, 3);
 *       if(HAL_RTC_STATUS_OK != ret) {
 *           //error handling
 *       }
 * @endcode
