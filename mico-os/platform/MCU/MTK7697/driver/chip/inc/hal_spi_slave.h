 ...
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux, if the EPT tool hasn't been used to configure the related pinmux.
 *           For more details about #hal_pinmux_set_function(), please refer to @ref GPIO.
 *  - Step2: Call #hal_spi_slave_register_callback() to register a user callback.
 *  - Step3: Call #hal_spi_slave_set_command() to set a user-defined command that matches the SPI master user-defined command. Apply this function to avoid the default settings.
 *  - Step4: Call #hal_spi_slave_set_early_miso() to change the SPI timing.
 *  - Step5: Call #hal_spi_slave_init() in the callback function, if Power-On interrupt occurred, to initialize a single SPI slave controller.
 *  - Step6: Call #hal_spi_slave_send() in the callback function, if configure read finish interrupt occurred, to send data in the DMA mode.
 *  - Step7: Call #hal_spi_slave_receive() in the callback function, if configure write finish interrupt occurred, to receive data in the DMA mode.
 *  - Step8: Call #hal_spi_slave_deinit() in the callback function, if Power-Off interrupt occurred, to deinitialize the SPI slave that is no longer in use.
 *  - sample code:
 *    @code
 *       hal_pinmux_set_function(gpio_pin, function_index);// No need to configure the pins, if the EPT tool is used.
 *       hal_spi_slave_register_callback(slave_port,user_spi_callback,NULL)// Register a user callback.
 *       ret = hal_spi_slave_set_command(slave_port,command,value); //Set a command value if needed.
 *       if (HAL_SPI_SLAVE_STATUS_OK != ret) {
 *          //Error handler;
 *       }
 *       ret = hal_spi_slave_set_early_miso(slave_port,early_miso); //Change the SPI timing if needed.
 *       if (HAL_SPI_SLAVE_STATUS_OK != ret) {
 *           //Error handler;
 *       }
 *    @endcode
 ...
 *       {
 *           hal_spi_slave_status_t ret_status = HAL_SPI_SLAVE_STATUS_OK;
 ...
 *                       if (HAL_SPI_SLAVE_STATUS_OK != ret_status) {
 *                           // Error handler;
 *                       }
 *                       break;
 *                   case SPISLV_IRQ_POWEROFF_IRQ_MASK:
 *                       ret_status = hal_spi_slave_deinit(HAL_SPI_SLAVE_0);
 *                       if (HAL_SPI_SLAVE_STATUS_OK != ret_status) {
 *                           // Error handler;
 *                       }
 *                       break;
 *                   case SPISLV_IRQ_CRD_FINISH_IRQ_MASK:
 *                       ret_status = hal_spi_slave_send(HAL_SPI_SLAVE_0, data, size);
 *                       if (HAL_SPI_SLAVE_STATUS_OK != ret_status) {
 *                           // Error handler;
 *                       }
 *                       break;
 *                   case SPISLV_IRQ_CWR_FINISH_IRQ_MASK:
 *                       ret_status = hal_spi_slave_receive(HAL_SPI_SLAVE_0, buffer, size);
 *                       if (HAL_SPI_SLAVE_STATUS_OK != ret_status) {
 *                           // Error handler;
 *                       }
 *                       break;
 ...
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux, if the EPT tool wasn't used to configure the related pinmux.
 *           For more details about #hal_pinmux_set_function(), please refer to @ref GPIO.
 *  - Step2: Call #hal_spi_slave_init() to configure the SPI slave hardware settings.
 *  - Step3: Call #hal_spi_slave_register_callback() to register a user callback.
 *  - An example implementation of the SPI slave.
 *    @code
 *       hal_pinmux_set_function(HAL_GPIO_29, HAL_GPIO_29_SPI_MOSI_S_CM4);// No need to configure pinmux, if EPT tool is used.
 *       hal_pinmux_set_function(HAL_GPIO_30, HAL_GPIO_30_SPI_MISO_S_CM4);// No need to configure pinmux, if EPT tool is used.
 *       hal_pinmux_set_function(HAL_GPIO_31, HAL_GPIO_31_SPI_SCK_S_CM4); // No need to configure pinmux, if EPT tool is used.
 *       hal_pinmux_set_function(HAL_GPIO_32, HAL_GPIO_32_SPI_CS_0_S_CM4);// No need to configure pinmux, if EPT tool is used.
 *       hal_spi_slave_config_t spi_configure;
 *       spi_configure.phase = HAL_SPI_SLAVE_CLOCK_PHASE0;
 *       spi_configure.polarity = HAL_SPI_SLAVE_CLOCK_POLARITY0;
 *       ret_status = hal_spi_slave_init(HAL_SPI_SLAVE_0, &spi_configure);
 ...
 *          service routine if an interrupt is triggered. For more details about the callback function, please refer to #spi_slave_register_callback().
 *  @param[in] user_data is a user defined parameter obtained from #spi_slave_register_callback().
 */
