 ...
 * @addtogroup G2D
 * @{
 * This section introduces the G2D (Graphics 2D) driver APIs including terms and acronyms, supported features, software architecture, details on how to use this driver, G2D Manager function groups, enums, structures and functions.
 *
 ...
 * Once the layer is configured, call the function #hal_g2d_overlay_start() to start the G2D engine. The layer0~3 will overlay to destination buffer.\n
 *
 ...
 * - Related functions\n
 * Call the function #hal_g2d_rectfill_set_destination_window() to set the destination, then call the function #hal_g2d_rectfill_set_color() to set the filling color.\n
 * To start G2D engine by using #hal_g2d_rectfill_start()\n
 *\n
 ...
 *  - Step3: Call the start functions such as #hal_g2d_overlay_start() and #hal_g2d_rectfill_start().
 ...
 *
 *  - Sample code: (The full source code can be found at project\mt2523_hdk\hal_examples\g2d_overlay_layer\src\g2d_test.c)
 *    @code
 *    hal_g2d_handle_t *g2dHandle;
 *    hal_g2d_get_handle(&g2dHandle, G2D_CODEC_TYPE_HW, HAL_G2D_GET_HANDLE_MODE_BLOCKING);
 *    hal_set_rgb_buffer(g2dHandle, (uint8_t *)dstBuf, 120*120*4, dstRectW, dstRectH, dstColorFormat);
 *    hal_g2d_replace_color(g2dHandle, false, 0, 255, 0, 0, 0, 0, 0, 255);
 *    hal_g2d_set_clipping_window(g2dHandle, false, 0, 0, dstRectW, dstRectH);
 *    hal_g2d_set_source_key(g2dHandle, false, 0, 0, 0, 0);
 *    hal_g2d_rectfill_set_destination_window(g2dHandle, 0, 0, dstRectW, dstRectH);
 *    hal_g2d_rectfill_set_color(g2dHandle, 0x12345678);
 *    hal_g2d_rectfill_start (g2dHandle);
...
typedef enum {
    ...
    HAL_G2D_CODEC_TYPE_SW       /**< Software codec. */
...
typedef enum {
    HAL_G2D_COLOR_FORMAT_GRAY = 0,                 /**< Gray. */
    HAL_G2D_COLOR_FORMAT_BGR888 = 1,               /**< BGR888. */ 
    HAL_G2D_COLOR_FORMAT_RGB565 = 2,               /**< RGB565. */
    HAL_G2D_COLOR_FORMAT_RGB888 = 3,               /**< RGB888. */
    HAL_G2D_COLOR_FORMAT_ARGB8888 = 4,             /**< ARGB8888. */
    HAL_G2D_COLOR_FORMAT_PARGB8888 = 5,            /**< PARGB8888. */
    HAL_G2D_COLOR_FORMAT_ARGB8565 = 6,             /**< GARGB8565. */
    HAL_G2D_COLOR_FORMAT_ARGB6666 = 7,             /**< ARGB6666. */ 
    HAL_G2D_COLOR_FORMAT_PARGB8565 = 8,            /**< PARGB8565. */
    HAL_G2D_COLOR_FORMAT_PARGB6666 = 9,            /**< PARGB6666. */
    HAL_G2D_COLOR_FORMAT_UYVY422 = 10,             /**< UYVY422.*/ 
    HAL_G2D_COLOR_FORMAT_ARGB8888_INDEX_COLOR = 11,/**< ARGB8888 index color. */ 
    HAL_G2D_COLOR_FORMAT_ARGB4444 = 12,            /**< ARGB444. */ 
...
typedef enum {
    ...
    HAL_G2D_SUPPORTED_FUNCTION_NORMAL_FONT,              /**< Normal font function. */
    HAL_G2D_SUPPORTED_FUNCTION_AA_FONT,                  /**< AA font function. */
    HAL_G2D_SUPPORTED_FUNCTION_TILT_FONT,                /**< Tilt font function. */
    HAL_G2D_SUPPORTED_FUNCTION_LT,                       /**< LT(Linear transform) function. */
    HAL_G2D_SUPPORTED_FUNCTION_SAD,                      /**< SAD(Sum as difference) function. */ 
...
typedef enum {
    HAL_G2D_OVERLAY_LAYER_FUNCTION_NORMAL_FONT = 0, /**< Normal font.*/
    HAL_G2D_OVERLAY_LAYER_FUNCTION_AA_FONT,         /**< AA font.*/
...
typedef enum {
    ...
    HAL_G2D_ROTATE_ANGLE_90,        /**< Rotate 90 degrees clockwise. */
    HAL_G2D_ROTATE_ANGLE_180,       /**< Rotate 180 degrees clockwise. */
    HAL_G2D_ROTATE_ANGLE_270,       /**< Rotate 270 degrees clockwise. */
    HAL_G2D_ROTATE_ANGLE_MIRROR_0,  /**< Mirror */
    HAL_G2D_ROTATE_ANGLE_MIRROR_90, /**< Mirror and then rotate 90 degrees clockwise. */
    HAL_G2D_ROTATE_ANGLE_MIRROR_180,/**< Mirror and then rotate 180 degrees clockwise.*/
    HAL_G2D_ROTATE_ANGLE_MIRROR_270 /**< Mirror and then rotate 270 degrees clockwise. */
...
/** @brief Defines the prototype of the G2D callback function. The callback is invoked whenever the G2D engine completes an operation, such as draws an overlay, makes a rotation or fills a rectangle. \n
     Example code.\n
	 extern void DrawLayerToDisply(); \n
        hal_g2d_register_callback(g2dHandle, DrawLayerToDisply);\n
        ...hal_g2d_overlay_start(g2dHandle);\n
     The DrawLayerToDisplay() function will be called after the hal_g2d_overlay process is complete.\n
*/
...
/**
 * @brief This function gets a G2D handle to access and control the G2D engine.
 * @param[out] handle_ptr is the pointer to the G2D handle, after the function returns successfully.
 * @param[in] codec_type specifies the G2D codec type. Usually, the #HAL_G2D_CODEC_TYPE_HW is used.
 * @param[in] handle_mode specifies whether to use blocking mode or non-blocking mode to get the G2D handle. If in blocking mode, this function will not return until G2D is no longer busy.
 * @return
...
/**
 * @brief This function releases the G2D handle and frees its associated resources and deallocates the memory pointed to by this G2D handle.
...
hal_g2d_status_t hal_g2d_replace_color(hal_g2d_handle_t *handle,
        uint8_t alpha0, uint8_t red0, uint8_t green0, uint8_t blue0,
        uint8_t alpha1, uint8_t red1, uint8_t green1, uint8_t blue1);


/**
 * @brief This function sets the source key value.
 * @param[in] handle specifies the G2D handle.
...
hal_g2d_status_t hal_g2d_overlay_enable_layer(hal_g2d_handle_t *handle,
        hal_g2d_overlay_layer_t layer, bool show_layer);
...
/**
 * @brief This function sets buffer for a layer.
 * @param[in] handle     specifies the G2D handle.
...
/**
 * @brief This function sets YUV buffer for a layer.
 * @param[in] handle        specifies the G2D handle.
...
/**
 * @brief This function sets the rectangle fill region.
 * @param[in] handle specifies the G2D handle.
...
/**
 * @brief This function sets the color for the rectangle fill.
 * @param[in] handle specifies the G2D handle.
 * @param[in] color  is the color to fill the rectangle. If the color gradient is disabled,  \n
 *                   all the specified rectangle region will be filled with the specified color. \n
 *                   If the color gradient is enabled (the gradient value is not zero),\n
 *                   the specified is the start color for rectangle gradient fill.
 * @return
 * #HAL_G2D_STATUS_OK, if the operation is successful.
 */
...
/**
 * @brief This function queries the range of the destination window.
 * @param[in] handle  specifies the G2D handle.
...
/**
 * @brief This function starts the rectangle fill operation.
 * @param[in] handle specifies the G2D handle.
 * @return
 * #HAL_G2D_STATUS_OK, if the operation is successful. \n
 * #HAL_G2D_STATUS_BUSY, if the G2D hardware is busy.
