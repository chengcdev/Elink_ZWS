 ...
 *  - sample code:
 *    @code
 *       ret_reset_status = hal_wdt_get_reset_status(); //get reset status first, because hal_wdt_init, hal_wdt_enable, hal_wdt_disable will clear the reset status. User can get the reason of last reset by this status. ret_reset_status may be used at any time after #hal_wdt_init() is called.
 *       wdt_config.mode = HAL_WDT_MODE_RESET;
 *       wdt_config.seconds = 4;
 *       ret = hal_wdt_init(&wdt_config); //set WDT as t0 mode.
 ...
 *       wdt_config.seconds = 4;
 *       ret = hal_wdt_init(&wdt_config); //set WDT as interrupt mode.
 *       if(HAL_WDT_STATUS_OK != ret) {
 *             //error handle
 *       }
 *       ret_callback = hal_wdt_register_callback(user_callback); // register a user callback.
 *       hal_wdt_enable(HAL_WDT_ENABLE_MAGIC); //enable WDT to start the timer.
 *       hal_wdt_feed(HAL_WDT_FEED_MAGIC);  //feed the WDT regularly.
 *       //...
 *       //hal_wdt_disable(HAL_WDT_DISABLE_MAGIC); //if you want to disable the WDT
...
/**
 * @brief This function is used to kick the watchdog.
