 ...
 * @addtogroup GPC
 * @{
 * This section introduces the GPC APIs including terms and acronyms,
 ...
 *
 * This module is a generic design to capture the number of pulse waveforms.
 *
 ...
 *  - Step1: Call #hal_pinmux_set_function to set GPC pinmux. For mode details about #hal_pinmux_set_function please refer to @ref GPIO module in HAL.
 *  - Step2: Call #hal_gpc_get_running_status() to get the current running status.
 *  - Step3: Call #hal_gpc_init() to configue the GPC hardware.
 *  - Step4: Call #hal_gpc_enable() to enable the GPC module.
 ...
 *       hal_gpc_port_t           gpc_port;
 *       uint32_t                 count_value;
 *
 *       hal_pinmux_set_function(gpio_pin, function_index);                 //set pinmux
 *
 *       gpc_port = HAL_GPC_PORT_0;
 *       hal_gpc_get_running_status(HAL_GPC_PORT_0,&running_status);        //get running status to check if this port is in use or idle.
 *       if ( running_status != HAL_GPC_STATUS_IDLE ) {                     //if GPC is running, exception handle
 *          //exception handle
 *       }
 *       ret_status = hal_gpc_init(gpc_port);                               //initialize GPC hardware with driver default configuration
 *       if(HAL_GPC_STATUS_OK != ret_status) {
 *          //error handle
 *       }
 *       hal_gpc_enable(gpc_port);                                          //enable GPC module
 *       //....                                                             //capture pulse
 *       //....
 *       hal_gpc_get_and_clear_count(gpc_port,&count_value);                //get counter value and clear it.
 *       hal_gpc_disable(gpc_port);                                         //enable GPC module
 *       hal_gpc_deinit(gpc_port);                                          //if no longer in use.
 *    @endcode
 * @}
 * @}
 */
...
*  - Step1: Call #hal_pinmux_set_function to set the GPC pinmux. For more details about #hal_pinmux_set_function, please refer to @ref GPIO API document.
*  - Step2: Call #hal_gpc_get_running_status() to get the current running status.
*  - Step3: Call #hal_gpc_init() to configrue the GPC hardware.
*  - Step4: Call #hal_gpc_advanced_config() to set parameter.
*  - Step5: Call #hal_gpc_register_callback() to register user's callback function.
*  - Step6: Call #hal_gpc_enable() to enable the GPC module.
*  - Step7: Call #hal_gpc_get_and_clear_count() to get GPC counter value.
*  - Step8: Call #hal_gpc_deinit() to de-initilaize the GPC module if GPC no longer in use.
*  - Sample code:
*    @code
*       hal_gpc_running_status_t running_status;
*       hal_gpc_status_t         ret_status;
...
*          //exception handle
*       }
*       ret_status = hal_gpc_init(gpc_port);                               //set GPC hardware.
*       if(HAL_GPC_STATUS_OK != ret_status) {
*          //error handle
*       }
*       hal_gpc_register_callback(gpc_port, user_gpc_callback,&user_data); //register a user callback.
*
...
*       hal_gpc_advanced_config(gpc_port,&gpc_advanced_config);
*
*       hal_gpc_interrupt_enable(gpc_port);                                //enable interrupt
*       hal_gpc_enable(gpc_port)                                           //enable GPC module
*       //....                                                             //capture pulse
*       hal_gpc_get_and_clear_count(gpc_port);                             //get counter value and clear, this step is usually implemented done in user's callback.
*
*       hal_gpc_interrupt_disble(gpc_port);                                //disable interrupt
*       gpc_advanced_config.threshold    =  200;
*       hal_gpc_set_threshold(gpc_port, &gpc_advanced_config.threshold);   //change threshold value as 200 means when counter reaches 200, an interrupt is triggered.
*       hal_gpc_enable(gpc_port)                                           //enable GPC module
*       //....                                                             //capture pulse
*
*    @endcode
...
*          hal_gpc_get_and_clear_count(gpc_port);                          //get counter value and clear, this step is usually implemented done in user's callback.
*          hal_gpc_disable(gpc_port);                                      //enable GPC module
*          hal_gpc_deinit(gpc_port);                                       //if it's no longer in use.
*       }
*    @endcode
* @}
* @}
...
/**
 * @brief     Enable the GPC module, after this GPC can capture pulses.
 * @param[in] gpc_port is the port number.
 * @return    #HAL_GPC_STATUS_OK, if OK. \n
 *            #HAL_GPC_STATUS_ERROR_PORT, the gpc_port value is wrong.
 * @sa        hal_gpc_disable().
...
/**
 * @brief     Disable the GPC module, after this GPC is unable to capture pulses.
 * @param[in] gpc_port is the port number.
 * @return    #HAL_GPC_STATUS_OK, if OK. \n
 *            #HAL_GPC_STATUS_ERROR_PORT, the gpc_port value is wrong.
 * @sa        hal_gpc_enable().
