 ...
 *    In extended DMA mode, the functions with the suffix "_dma_ex" return a value once the I2C and DMA hardware registers are assigned. These functions do not wait for the transaction to finish
 *    . After the transaction finishes, an interrupt is triggered and the user callback in the I2C ISR
 *    routine is called. After the function returns, the return value should be checked and the error status should be handled. 
 ...
 *  - Step1: Call #hal_pinmux_set_function() to set the GPIO pinmux or use the EPT tool to apply the pinmux settings.
 *  - Step2: Call #hal_i2c_master_init() to initialize the I2C master.
 ...
 *  - Step5: Call #hal_i2c_master_deinit() to de-allocate the I2C master if it is no longer in use.
 *  - Sample code:
 *    @code
 ...
 *  - Step4: Call #hal_i2c_master_send_dma() to send data within a DMA mode.
 *  - Step5: Call #hal_i2c_master_receive_dma() to receive data in a DMA mode.
 *  - Step6: Call #hal_i2c_master_deinit() to de-initialize the I2C master if it is no longer in use.
 *  - Sample code:
 *    @code
 *       hal_pinmux_set_function(gpio_pin, function_index);//Set the pinmux
 *       if(HAL_I2C_STATUS_OK == hal_i2c_master_init(i2c_port, &i2c_config)) {
 *           hal_i2c_master_register_callback(i2c_port, i2c_user_callback, (void *) &user_data)//Register a user callback.
 ...
 *  - Step2: Call #hal_i2c_master_init() to initialize the I2C master.
 *  - Step3: Call #hal_i2c_master_register_callback() to register a user callback.
 *  - Step4: Call #hal_i2c_master_send_dma_ex() to send data in the extended DMA mode.
 *  - Step5: Call #hal_i2c_master_receive_dma_ex() to receive data in the extended DMA mode.
 *  - Step6: Call #hal_i2c_master_deinit() to de-initialize the I2C master if it is no longer in use.
 *  - Sample code:
 *    @code
 ...
 *       if(HAL_I2C_STATUS_OK == hal_i2c_master_init(i2c_port, &i2c_config)) {
 *           hal_i2c_master_register_callback(i2c_port, i2c_user_callback, (void *) &user_data)// Register a user callback.
 *           ret = hal_i2c_master_send_dma_ex(i2c_port, &send_config); //Send data by calling the extended DMA API.
 *           if (HAL_I2C_STATUS_OK != ret) {
 *               //Error handling;
 *           }
 *       } else {
...
typedef enum {
    ...
    HAL_I2C_FREQUENCY_400K = 3,          /**<  400kbps. */
