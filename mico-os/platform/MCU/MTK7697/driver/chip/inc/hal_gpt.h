 ...
 * @addtogroup GPT
 * @{
 * This section introduces the GPT driver APIs including terms and acronyms, supported features, software architecture, details on how to use this driver, GPT function groups, enums, structures and functions.
 *
 ...
 *   Delay function is a hardware delay, it's for some programs to delay for a specified period without the need for the context switch.\n
 ...
 * @{
 * - \b Support \b delay \b function. \n
 *   The GPT driver supports delays in milliseconds #hal_gpt_delay_ms().
 *   It utilizes the free run mode of the timer and the polling mechanism to determine if the timeout for the timer is reached.\n
...
#ifdef HAL_GPT_PORT_ALLOCATE
/**
 * @addtogroup HAL
 * @{
 * @addtogroup GPT
 * @{
 * @section HAL_GPT_Driver_Port_Chapter Port allocation
 *
 * The GPT driver has 6 timer ports, some of the ports are used by the driver. The driver allocated port cannot be used for any other purpose.
 * - HAL_GPT_0 - Used for OSTD timer.
 * - HAL_GPT_1 - User defined. 
 * - HAL_GPT_2 - User defined. 
 * - HAL_GPT_3 - Used for software GPT.
 * - HAL_GPT_4 - Use to set a microsecond delay and get microsecond free count.
 * - HAL_GPT_5 - Use to set a millisecond delay and get 1/32Khz free count.
 *
 * @}
 * @}
 */
 ...
 *  - Step4: Call #hal_gpt_stop_timer() to stop the timer.
 ...
 *       hal_gpt_get_running_status(HAL_GPT_PORT_0,&running_status);        //Get the running status to check if this port is in use or not.
 *       if ( running_status != HAL_GPT_STOPPED ) {                         //Handle the exceptions, if the timer is running.
 *          //exception handler
 *       }
 ...
 *       {
 *           // user's handler
 *           hal_gpt_stop_timer(gpt_port);                                    //Stop the timer.
 ...
 * @addtogroup GPT
 * @{
 *  - For the delay in microseconds, the #hal_gpt_start_timer_us() works the same way as with #hal_gpt_start_timer_ms().
 * @}
 * @}
 */
#endif
 ...
 *       }
 *
 *       hal_gpt_delay_us(10);                       //Delay for 10 ms.
 *
 *       //Get the second value.
 ...
 *       uint32_t                 handle;
 *
 *       ret_status = hal_gpt_sw_get_timer(&handle); //Get the allocated timer handle
 *       if (ret_status != HAL_GPT_STATUS_OK) {                          
 *          //error handler
 *       }
 *
 *       //Set the timeout to 10 ms, and register the callback at the same time
 *       ret_status = hal_gpt_sw_start_timer_ms(handle, 10, user_gpt_callback, NULL);                               
 *       if(ret_status != HAL_GPT_STATUS_OK) {
 *          //error handler
 *       }
 *       
 ...
 *            The function should only be called while the timer is stopped. Calling this function when the timer is running would result in an unexpected behavior.
 * @param[in] gpt_port is the port number.
 ...
 *            The function should only be called while the timer is stopped. Calling this function when the timer is running would result in unexpected behavior.
 * @param[in] gpt_port is the port number.
...
/**
 * @brief      This function calculates the count duration.
