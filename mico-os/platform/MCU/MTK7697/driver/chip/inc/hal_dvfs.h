 ...
 *   - Step1: Call #hal_dvfs_init() to initialize the DVFS module and only need to call this once at system initialization.
 *   - Step2: Call #hal_dvfs_get_cpu_frequency() to get current CPU frequency.
 *   - Step3: Call #hal_dvfs_target_cpu_frequency() to adjust the current CPU frequency.
 *   - Step4: Call #hal_dvfs_get_cpu_frequency() to get current CPU frequency.
 *   - Step5: Print out the frequencies obtained from Step2 and Step4 to identify the difference.
 *   - Sample code:
 *   @code
 *
 *       uint32_t old_freq, cur_freq;
 *
 *       hal_dvfs_init();
 *       old_freq = hal_dvfs_get_cpu_frequency();
 *       hal_dvfs_target_cpu_frequency(26000, HAL_DVFS_FREQ_RELATION_L); // Change the CPU frequency to 26MHz.
...
/** @defgroup hal_dvfs_enum Enum
  * @{
  */
/** @brief This enum defines return status of certain DVFS HAL APIs. User should check return value after calling some APIs. */
typedef enum {
    ...
    HAL_DVFS_STATUS_INVALID_PARAM  = -2,      /**< User's input parameter is invalid. */
 ...
 * @param[in]   target_freq, the target frequency in kHz.
 ...
 *              #HAL_DVFS_STATUS_INVALID_PARAM, if user's parameter is invalid.\n
 *              #HAL_DVFS_STATUS_ERROR, if other errors occurred.\n
 */
...
/**
 * @brief       This function gets all adjustable CPU frequencies.
 * @param[out]   list: double pointer to the frequency table.
 * @param[out]   list_num: pointer to the number of frequency table.
