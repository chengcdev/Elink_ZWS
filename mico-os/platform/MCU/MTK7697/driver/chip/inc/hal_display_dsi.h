 ...
 * |\b HS                       | High speed mode. High speed mode enables the high speed clock that acts as the bit clock for the data lanes. High speed mode is still designed to reduce power usage due to its low voltage signaling and parallel transfer ability.
 ...
 *  - Step1: Call #hal_display_lcd_init() and #hal_display_dsi_init() to initialize the LCD and DSI hardware.
 ...
 *  - Step4: Call blockwrite function from the LCM driver function table to start the transfer.
 *  - sample code:
 *    @code
...
typedef enum {
    ...
    HAL_DISPLAY_DSI_TRANSFER_MODE_HS,                   /**< The DSI always transfers in high speed mode. */
    HAL_DISPLAY_DSI_TRANSFER_MODE_AUTO,                 /**< The DSI will switch to low power / high speed mode automatically according to the command type. */
 ...
 */
typedef struct {
    uint32_t    da_hs_trail;                /**< Time that the transmitter drives the flipped differential state after last payload data bit of a HS transmission burst. */
    uint32_t    da_hs_zero;                 /**< Time that the transmitter drives the HS-0 state prior to transmit the Sync sequence. */
    uint32_t    da_hs_prep;                 /**< Time that the transmitter drives the Data Lane LP-00 Line state immediately before the HS-0 Line state starting the HS transmission. */
    uint32_t    lpx;                        /**< Transmitted length of any Low-Power state period to display module. */
    uint32_t    da_hs_exit;                 /**< Time that the transmitter drives LP-11 following a HS burst. */
    ...
    uint32_t    clk_hs_trail;               /**< Time that the transmitter drives the HS-0 state after the last payload clock bit of a HS transmission burst. */
 ...
 *
 * @param[in] ssc_enable specifies whether the spread spectrum is enabled or not.
 *
 ...
 * @param[in] forced_update represents to send data to LCM driver IC immediately or not.
 *
 * @return      To indicate whether this function call is successful or not.
 *              #HAL_DISPLAY_DSI_STATUS_OK, the operation completed successfully.
 ...
 *
 * @param[in] pll_clock is the PLL clock speed in hertz.
...
/**
 * @brief This function starts the BTA type transfer. Add this function at the end of LCM blockwrite function to start transferring data, to use the internal TE.
 *
